...
def transform_list_prod_unit(execution_date, **kwargs):
    ...
    current_day = datetime.date(current_day)
    #execution_date = datetime.date(execution_date - timedelta(days=1))
    data = read_extract_data(current_day)
    ...
    try:
        ...
        list_difference = transform(
            production_unit(1, "", 1), set(data_only_prod_unit), get_production_unit_sql_query_from_dst.format(greenhouse=greenhouse))
...
def transform_list_topic(execution_date, **kwargs):
    ...
    try:
        ...
        list_difference = transform(
            topic(1, ""), set(data_only_topic), get_topic_sql_query_from_dst.format(greenhouse=greenhouse))
        # print(len(list_difference))
        ...
        write_transform_data_to_json(topic(1, ""), unit_list, current_day)
    except Exception as e:
        log.error(
...
def transform_list_clients(execution_date, **kwargs):
    ...
    try:
        ...
        list_difference = transform(
            clients(1, ""), set(data_only_client), get_client_sql_query_from_dst.format(greenhouse=greenhouse))
        print(len(list_difference))
        ...
        try:
            ...
            log.error(
                "Ошибка создания списка json. Error: {err}", err=e)
            send_alert(e, error_head + sys._getframe().f_code.co_name)
        write_transform_data_to_json(clients(1, ""), unit_list, current_day)
    except Exception as e:
        log.error(
    ...
    try:
        for item in data:
            try:
                ...
                continue
        log.info(len(data_only_base))

        list_difference = transform(
            bases(1, "", 1), set(data_only_base), get_base_sql_query_from_dst.format(greenhouse=greenhouse))
        log.info("Итоговый список для записи" + str(len(list_difference)))
        try:
            if list_difference is not None and len(list_difference) > 0:
                with dst.get_conn() as conn:
                    with conn.cursor() as cur:
        ...
        write_transform_data_to_json(
            bases(1, "", 1), unit_list, current_day)
    except Exception as e:
    ...
    try:
        for item in data:
            try:
                ...
                continue
        log.info("Результат анализа датасета: " + str(len(data_only_sensors)))

        list_difference = transform(
            sensor(1, "", 1), set(data_only_sensors), get_sensor_sql_query_from_dst.format(greenhouse=greenhouse))
        log.info(len(list_difference))
        ...
        try:
            ...
            send_alert(e, error_head + sys._getframe().f_code.co_name)
        write_transform_data_to_json(
            sensor(1, "", 1), unit_list, current_day)
    except Exception as e:
        log.error(
...
def transform_list_sensors_value(execution_date, **kwargs):
    ...
    try:
                        ...
                        try:
                            for sensor in list_sensors:
                                if sensor[0] == _sensor:
                                    ...
                                    break
                            if val.id_sensor == -1:
                                log.info("Сенсор: " + str(_sensor) +
                                         " из " + str(item[2]))
                                query = get_id_sensor_from_name_sql_query_from_dst.format(
                                    name=_sensor, greenhouse=greenhouse)
                                log.info(query)
                                cur.execute(query)
                                result = cur.fetchone()
        ...
        write_transform_data_to_json(
            sensor_value(-1), data_only_value, current_day)
...
def finnaly_extract(execution_date, **kwargs):
    ...
    # ti.xcom_pull(key='execution_date')
    current_day = execution_date - timedelta(minutes=3)
    execution_date = datetime.date(current_day)
    try:
